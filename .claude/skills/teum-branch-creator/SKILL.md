...
선택된 브랜치명의 type으로 올바른 베이스를 결정한다 (`hotfix` → `main`, 그 외 → `develop`).
현재 브랜치가 올바른 베이스와 다르면 아래와 같이 확인 후 전환한다.

```
이 브랜치는 `<올바른 베이스>` 를 기준으로 생성해야 합니다.
현재 브랜치: `<현재 브랜치명>`

`<올바른 베이스>` 로 전환한 뒤 최신 상태로 진행할까요?
...
현재 브랜치가 이미 올바른 베이스와 같으면 이 단계를 건너뛴다.
...
- `hotfix`가 아닌 type이 `main`을 베이스로 하려는 경우 → STEP 4.5에서 `develop`으로 전환 안내
- `hotfix` type이 `develop`을 베이스로 하려는 경우 → STEP 4.5에서 `main`으로 전환 안내

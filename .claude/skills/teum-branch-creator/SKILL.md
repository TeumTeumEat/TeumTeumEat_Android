...
현재 브랜치를 베이스로 사용할 것임을 사용자에게 알린다.
...
출력 형식:
```
아래 3가지 브랜치명을 추천합니다. (베이스: <현재 브랜치명>)
...
사용자가 브랜치명을 선택(또는 입력)하면 아래를 실행한다.
...
- `main` 브랜치를 베이스로 사용할 경우 → 경고 후 사용자 재확인
  - 일반적으로 `develop_*` 또는 기능 브랜치에서 분기하도록 권장

...
번호를 입력하거나 브랜치명을 직접 입력해주세요. (기본값: develop)
```

`feature`/`fix` 등 일반 개발 브랜치는 `develop`으로 병합하는 것이 기본 흐름이다.
`release`는 QA 배포 시점에 프로젝트 담당자가 `develop`을 병합하는 브랜치이며,
`main`은 아래 STEP 1.4 게이트를 통과해야만 진행할 수 있다.

사용자가 브랜치를 선택하면 이후 모든 단계에서 해당 브랜치를 베이스 브랜치(`BASE_BRANCH`)로 사용한다.

### STEP 1.4 — `main` 브랜치 게이트 (`BASE_BRANCH`가 `main`일 때만)

`main`은 언제나 배포 가능한 상태를 유지해야 하는 실서버 배포 브랜치이며,
**어떤 상황에서든 직접 PR·머지를 진행하지 않는 것이 원칙**이다.
`BASE_BRANCH`가 `main`이 아니면 이 단계 전체를 건너뛰고 STEP 2로 이동한다.

불가피하게 `main`으로 진행해야 한다면 아래 순서로 확인하고,
하나라도 통과하지 못하면 진행을 중단하고 STEP 1로 돌아가 `BASE_BRANCH`를 다시 확인한다.

**1) 진행 여부 확인**

```
main은 언제나 배포 가능한 상태를 유지해야 하는 배포 브랜치입니다.
어떤 상황에서든 main에 직접 PR·머지를 진행하지 않는 것이 원칙입니다.

정말 main으로 PR을 진행하시겠습니까?

[y] 진행   |   [n] 취소 (다른 브랜치로 변경)
```

`n` 또는 거부 응답 시 STEP 1로 돌아가 `BASE_BRANCH`를 재확인한다.

**2) 작업 이유 확인**

```
main으로 진행해야 하는 이유를 알려주세요.
(예: 정식 배포 준비 완료, 프로덕션 긴급 hotfix 대응 등)
```

**3) 타당성 검증**

아래 기준으로 응답의 타당성을 판단한다.

- 실제로 배포 가능한 상태(빌드·테스트 통과, 기능 완결)인가
- 정식 릴리즈 또는 `hotfix/*` 긴급 수정처럼 `main` 직행이 합리적인 상황인가
- 단순 기능 개발·중간 작업물을 `main`으로 착각한 경우가 아닌가

**타당하지 않다고 판단되면** 판단 근거를 설명하고 `develop` 또는 `release` 등 적절한 브랜치로 변경할 것을 권장한 뒤 진행을 중단한다.
**타당하다고 판단되면** 판단 근거를 사용자에게 알리고 STEP 1.5로 진행한다.

`hotfix/*` 브랜치가 `main`에 병합된 경우, 병합 완료 후 동일 수정을 `develop`(진행 중인 `release`가 있다면 `release`에도)에
백포트하도록 사용자에게 안내한다 — 그렇지 않으면 다음 정식 배포에서 동일 오류가 재발할 수 있다.
...
- `BASE_BRANCH`가 `main`인 경우 → STEP 1.4 게이트(진행 여부·사유 확인 → 타당성 검증) 통과 전까지 진행 금지
